...
def show_gene_percentage():
    ...
    curdoc().add_next_tick_callback(lambda : next_show(buttons_group))
...
def basic_filter_callback(input1, input2):
    ...
    def next_filter(buttons_group, input1, input2):
        ...
        button_abled(buttons_group)
    curdoc().add_next_tick_callback(lambda : next_filter(buttons_group, input1, input2))
...
def mitochondrial_genes():
    ...
    def next_mit(buttons_group):
        ...
        change.set_anndata(adata)
        button_abled(buttons_group)
    curdoc().add_next_tick_callback(lambda : next_mit(buttons_group))
...
def filter_norm_log(gene_counts, mt_pct, target_sum):
    ...
    def next_filter(buttons_group, gene_counts, mt_pct, target_sum):
        ...
        change.set_anndata(adata)
        button_abled(buttons_group)
    curdoc().add_next_tick_callback(lambda : next_filter(buttons_group, gene_counts, mt_pct, target_sum))
...
def hvg(min_mean, max_mean, min_disp):
    ...
    def next_hvg(buttons_group, min_mean, max_mean, min_disp):
        ...
        layout.children.append(div)
        button_abled(buttons_group)
    curdoc().add_next_tick_callback(lambda : next_hvg(buttons_group, min_mean, max_mean, min_disp))
