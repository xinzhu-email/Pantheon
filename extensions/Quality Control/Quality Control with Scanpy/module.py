...
class PanelExt():
    def __init__(self, func_name, ext_name):
        ...
        self.data = AnnData()
        self.result_dict = {
            DataCat.HDCoordinate.value: list[int](),
            DataCat.LDCoordinate_obs.value: list[int](),
            DataCat.LDCoordinate_var.value: list[int](),
            DataCat.Catagorial_obs.value: list[int](),
            DataCat.Catagorial_var.value: list[int](),
            DataCat.Data_obs.value: list[int](),
            DataCat.Data_var.value: list[int](),
            DataCat.Pairwise_obs.value: list[int](),
            DataCat.Pairwise_var.value: list[int](),
            DataCat.Unstructured.value: list[int](),
        }
    ...
    def prefix_exec_callback(self):
        print(self.data)
        prefix_label = self.widgets_dict['prefix_label'].value
        prefix_value = self.widgets_dict['prefix_value'].value
        print(prefix_label, prefix_value)
        if self.widgets_dict['prefix_label'].value not in list(self.prefix.keys()):
            self.prefix[prefix_label] = prefix_str2list(prefix_value)
        else:
            self.prefix[prefix_label] += prefix_str2list(prefix_value)
        labels = []
        active = []
        ...
        for i in range(len(prefix_keys)):
            ...
            if key in active_prefix_labels:
                active.append(i)
        self.data.var[prefix_label] = self.data.var_names.str.startswith(prefix_label)
        ...
        self.result_dict[DataCat.Catagorial_var.value] = prefix_keys + self.result_dict[DataCat.Catagorial_var.value]
        print(self.data.var)
    ...
    def prefix_qc_callback(self):
        active_prefix_labels = [self.widgets_dict['prefix_dspl'].labels[i].split(":")[0] for i in self.widgets_dict['prefix_dspl'].active]
        print(active_prefix_labels)
        for prefix_label in active_prefix_labels:
            prefix_tuple = tuple(self.prefix[prefix_label])
            self.data.var[prefix_label] = self.data.var_names.str.startswith(prefix_tuple)
        
        sc.pp.calculate_qc_metrics(
            self.data, qc_vars = active_prefix_labels, inplace=True, log1p=False
        )
    ...
    """functional"""
    def add_results(self, result_type: DataCat, variable_name: str):
        self.result_dict[result_type.value].append(variable_name)