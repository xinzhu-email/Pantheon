...
def load_options():
    try:
        path = os.path.split(os.path.abspath(__file__))[0] # __file__ means current file (with abspath making sure it's absolute path)
        print(path)
        name_list = os.listdir(path+'/extension')
...
def upload_callback(upload_button): # callback function was asked by another function
    ...
    path = os.path.split(__file__)[0] # split the path and the filename by the last '/'
                                      # [0] means the first tuple element: path not the filename
    def load():
        global Main_plot
        filetype = os.path.splitext(upload_button.filename)[-1][1:] # split the filename and the type
