...
def plot_r_umap():
    ...
    def plot_r(buttons_group):
        r('''
            ...
            new_data <- RunUMAP(data, nn.name = "weighted.nn", reduction.name = "wnn.umap", reduction.key = "wnnUMAP_")
            
            new_data@meta.data['label'] <- new_label[,'cell_type']
            new_data <- FindClusters(data, graph.name = "wsnn", algorithm = 3, resolution = 1, verbose = FALSE)
