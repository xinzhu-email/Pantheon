...
def rank(method,gene_num):
    ...
    def next_rank(buttons_group, method, gene_num):
        ...
        button_abled(buttons_group)
    curdoc().add_next_tick_callback(lambda : next_rank(buttons_group, method, gene_num))
...
def violin(gene_num):
    ...
    def next_violin(buttons_group, gene_num):
        ...
        for cluster in cluster_list[1:]:
            ...
            layout.children.append(div)
        button_abled(buttons_group)
    curdoc().add_next_tick_callback(lambda : next_violin(buttons_group, gene_num))
...
def compare(gene_list,group):
    ...
    def next_compare(buttons_group, gene_list, group):
        ...
        layout.children.append(div)
        button_abled(buttons_group)
    curdoc().add_next_tick_callback(lambda : next_compare(buttons_group, gene_list, group))